...
def wai(b):
	...
	for A in range(0,len(buf),6):
		...
		print('Thinking harder...');E=[think_harder]
		for A in D:
		  E.append(str(A))
...
def main():
	...
	D='rb';
	global buf;
	A=open(__file__,D).read();
	B=hashlib.md5();
	B.update(A);
	K=B.hexdigest()[:4];
	A=open(think_harder,D).read();
	B=hashlib.md5();
	B.update(A);
	L=B.hexdigest()[:4];
		...
		if H!=0:
			if H!=1:
				...
				if I[3]==61 and I[0]==44:
				  print('This is the flag:',open('/flag',D).read());
				  return 0
				else:
